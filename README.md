...
A **fully dynamic, settings-controlled developer portfolio** built with React 19, Vite, Tailwind CSS v4, and Framer Motion. Every section of the portfolio — from your name and skills to projects and certifications — is editable live through a built-in **settings Console**, with changes persisted via `db.json` and `localStorage`.
...
- **Animated sidebar navigation** — scrolls to active section; respects settings-controlled section visibility
...
│   ├── settings.jsx            # settings Console controller (state + handlers)
...
- **settings Console**: `http://localhost:5173/portfolio/settings`

---

## ⚙️ settings Console

Navigate to `/settings` on your deployed site or `http://localhost:5173/portfolio/settings` in dev.
...
1. Make changes in the settings Console
...
1. Visit `your-site.com/settings`
...
Or use the settings Console — **Theme & Colors** tab for live preview and save.
