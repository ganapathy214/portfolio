import React, { useEffect, useRef, useState, useCallback } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { FaLinkedinIn, FaGithub, FaInstagram, FaTwitter } from "react-icons/fa";
import { FiExternalLink, FiMail, FiChevronRight, FiMenu, FiX, FiInfo, FiCode, FiAward } from "react-icons/fi";
import { HiOutlineUser, HiBriefcase, HiEnvelope, HiAcademicCap } from "react-icons/hi2";
import avatar from "../assets/avatar.png";
import { downloadPdf } from "../utils/pdf";
import resumePdf from "../assets/resume.pdf";

const SOCIAL_ICONS = {
  FaLinkedinIn, FaGithub, FaInstagram, FaTwitter
};

export default function Template4Page({
  primaryColor = "#00D5D5",
  roles = [],
  description = "",
  about = {},
  skills = [],
  projects = [],
  certifications = [],
  timelineData = [],
  contactInfo = {},
  themeMode = "dark",
  sectionVisibility = {}
}) {
  const [activeSection, setActiveSection] = useState("about");
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
  const contentRef = useRef(null);

  const accent = primaryColor;
  const isDark = themeMode === "dark";

  // Dynamic Theme Colors
  const bgMain = isDark ? "#090d16" : "#f1f5f9";
  const bgCard = isDark ? "#0f1626" : "#ffffff";
  const bgNavbar = isDark ? "rgba(9, 13, 22, 0.9)" : "rgba(241, 245, 249, 0.9)";
  const textColor = isDark ? "#94a3b8" : "#475569";
  const textTitle = isDark ? "#ffffff" : "#0f172a";
  const textSub = isDark ? "#64748b" : "#64748b";
  const borderColor = isDark ? "rgba(255,255,255,0.07)" : "rgba(0,0,0,0.08)";

  const firstRole = roles?.[0] || about.title || "Frontend Developer";

  // Section observer
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            setActiveSection(entry.target.id);
          }
        });
      },
      { threshold: 0.2, rootMargin: "-10% 0px -50% 0px" }
    );
    const sections = document.querySelectorAll("[data-t4-section]");
    sections.forEach((s) => observer.observe(s));
    return () => sections.forEach((s) => observer.unobserve(s));
  }, []);

  const scrollTo = useCallback((id) => {
    const el = document.getElementById(id);
    if (el) {
      el.scrollIntoView({ behavior: "smooth", block: "start" });
    }
    setMobileMenuOpen(false);
  }, []);

  const visibleNav = [
    { id: "about", label: "About", icon: HiOutlineUser },
    { id: "experience", label: "Experience", icon: HiBriefcase, visibilityKey: "Journey" },
    { id: "formation", label: "Formation", icon: HiAcademicCap, visibilityKey: "Certification" },
    { id: "projects", label: "Projects", icon: FiCode, visibilityKey: "Projects" },
    { id: "contact", label: "Contact", icon: HiEnvelope, visibilityKey: "Contact" }
  ].filter((item) => {
    if (!item.visibilityKey) return true;
    return sectionVisibility[item.visibilityKey] !== false;
  });

  return (
    <div
      ref={contentRef}
      style={{
        backgroundColor: bgMain,
        color: textColor,
        minHeight: "100vh",
        fontFamily: "'Courier New', Courier, monospace, sans-serif",
        overflowX: "hidden"
      }}
    >
      <style>{`
        .t4-border-glow:hover {
          border-color: ${accent}80 !important;
          box-shadow: 0 0 15px ${accent}15;
        }
        .marquee-container {
          display: flex;
          overflow: hidden;
          white-space: nowrap;
          user-select: none;
        }
        .marquee-content {
          display: inline-flex;
          animation: loop-scroll 25s linear infinite;
        }
        @keyframes loop-scroll {
          from { transform: translateX(0); }
          to { transform: translateX(-50%); }
        }
      `}</style>

      {/* TOP NAVBAR */}
      <header
        style={{
          position: "sticky",
          top: 0,
          zIndex: 100,
          backgroundColor: bgNavbar,
          backdropFilter: "blur(12px)",
          WebkitBackdropFilter: "blur(12px)",
          borderBottom: `1px solid ${borderColor}`
        }}
      >
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          {/* Logo */}
          <button
            onClick={() => scrollTo("about")}
            className="text-lg font-black tracking-widest text-left cursor-pointer"
            style={{ color: textTitle }}
          >
            {(about.name || "Dev").split(" ")[0]} <span style={{ color: accent }}>/</span>
          </button>

          {/* Links */}
          <nav className="hidden md:flex items-center gap-8">
            {visibleNav.map((item) => (
              <button
                key={item.id}
                onClick={() => scrollTo(item.id)}
                className="text-xs font-bold uppercase tracking-widest transition-colors cursor-pointer"
                style={{
                  color: activeSection === item.id ? accent : textTitle
                }}
              >
                {item.label}
              </button>
            ))}
          </nav>

          {/* Mobile Toggle */}
          <button
            onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
            className="md:hidden p-2 text-xl cursor-pointer"
            style={{ color: textTitle }}
          >
            {mobileMenuOpen ? <FiX /> : <FiMenu />}
          </button>
        </div>

        {/* Mobile menu */}
        <AnimatePresence>
          {mobileMenuOpen && (
            <motion.div
              initial={{ opacity: 0, y: -10 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -10 }}
              className="md:hidden w-full absolute top-20 left-0 bg-stone-950 border-b border-stone-900 py-5 px-6 flex flex-col gap-4 text-left"
            >
              {visibleNav.map((item) => (
                <button
                  key={item.id}
                  onClick={() => scrollTo(item.id)}
                  className="text-xs font-bold uppercase tracking-widest text-left py-2"
                  style={{
                    color: activeSection === item.id ? accent : "#cbd5e1"
                  }}
                >
                  {item.label}
                </button>
              ))}
            </motion.div>
          )}
        </AnimatePresence>
      </header>

      {/* HERO SECTION */}
      <section
        id="about"
        data-t4-section
        className="min-h-[75vh] flex items-center py-16 px-6"
      >
        <div className="max-w-6xl mx-auto w-full grid grid-cols-1 md:grid-cols-12 gap-12 items-center">
          {/* Monospace Code tag illustration */}
          <div className="md:col-span-5 flex justify-center order-2 md:order-1">
            <div
              className="w-64 h-64 border-2 flex flex-col items-center justify-center relative rounded-3xl"
              style={{
                borderColor: `${accent}40`,
                boxShadow: `inset 0 0 30px ${accent}0a, 0 0 40px ${accent}08`
              }}
            >
              <div
                className="text-6xl font-black font-mono tracking-tighter"
                style={{ color: accent }}
              >
                &lt;/&gt;
              </div>
              <span className="text-[10px] uppercase font-mono tracking-[0.35em] text-stone-500 mt-4 block">
                dev workspace
              </span>
            </div>
          </div>

          {/* Intro Text */}
          <div className="md:col-span-7 text-left order-1 md:order-2 space-y-6">
            <span
              className="text-xs font-mono font-bold tracking-[0.25em] uppercase"
              style={{ color: accent }}
            >
              Hi, I'm
            </span>
            <h1
              className="text-4xl sm:text-5xl md:text-6xl font-black tracking-tight"
              style={{ color: textTitle, lineHeight: 1.1 }}
            >
              {about.name || "Dev"}
            </h1>
            <h2
              className="text-md sm:text-lg font-bold tracking-wide"
              style={{ color: textSub }}
            >
              {firstRole}
            </h2>
            <p className="text-xs sm:text-sm leading-relaxed max-w-xl">
              {description || about.bio || "Custom tailored coding and design architect."}
            </p>

            {/* Quick tech/social tags */}
            <div className="flex flex-wrap gap-2.5 pt-4">
              {(about.socialLinks || []).slice(0, 3).map((link, i) => {
                const Icon = SOCIAL_ICONS[link.icon] || FaGithub;
                return (
                  <a
                    key={i}
                    href={link.href}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-2 border px-4 py-2 rounded-xl text-[10px] font-bold uppercase tracking-wider t4-border-glow"
                    style={{ borderColor: borderColor, color: textTitle, backgroundColor: bgCard }}
                  >
                    <Icon className="text-xs" />
                    <span>{link.label}</span>
                  </a>
                );
              })}
            </div>
          </div>
        </div>
      </section>

      {/* MARQUEE BANNER ROW */}
      <div
        className="w-full py-5 border-y overflow-hidden select-none bg-stone-950/20"
        style={{ borderColor: borderColor }}
      >
        <div className="marquee-container text-xs font-mono font-bold uppercase tracking-[0.3em]" style={{ color: accent }}>
          <div className="marquee-content gap-16 pr-16">
            {Array(8).fill(
              skills.slice(0, 6).map(s => s.name).join(" / ") || "WEB / PROGRAMMING / DEVELOPMENT / JAVASCRIPT / CSS"
            ).map((txt, idx) => (
              <span key={idx}>{txt} /</span>
            ))}
          </div>
          <div className="marquee-content gap-16 pr-16" aria-hidden="true">
            {Array(8).fill(
              skills.slice(0, 6).map(s => s.name).join(" / ") || "WEB / PROGRAMMING / DEVELOPMENT / JAVASCRIPT / CSS"
            ).map((txt, idx) => (
              <span key={idx}>{txt} /</span>
            ))}
          </div>
        </div>
      </div>

      {/* ABOUT / STATS */}
      <section
        className="py-24 px-6 border-b"
        style={{ borderColor: borderColor }}
      >
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-12 gap-12 items-center">
          {/* Gradient Square box */}
          <div className="md:col-span-5 flex justify-center">
            <div
              className="w-64 h-64 relative rounded-3xl overflow-hidden group shadow-xl shrink-0"
              style={{ border: `1.5px solid ${accent}30` }}
            >
              {/* solid neon background overlay */}
              <div
                className="absolute inset-2 rounded-2xl opacity-80"
                style={{
                  background: `linear-gradient(135deg, ${accent}aa, #000000 90%)`,
                  filter: "blur(4px)"
                }}
              />
              <img
                src={about.avatarUrl || avatar}
                alt={about.name || "Developer Portrait"}
                className="absolute inset-2 w-[calc(100%-16px)] h-[calc(100%-16px)] object-cover rounded-2xl grayscale hover:grayscale-0 transition-all duration-500"
              />
            </div>
          </div>

          {/* Info Details */}
          <div className="md:col-span-7 text-left space-y-6">
            <h2 className="text-2xl font-black uppercase tracking-wider" style={{ color: textTitle }}>
              About
            </h2>
            <p className="text-xs sm:text-sm leading-relaxed">
              {about.bio || "Custom coding professional building efficient React apps, scalable APIs, and pixel perfect layouts."}
            </p>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
              {(about.highlights || []).map((h, i) => (
                <div key={i} className="flex gap-2 text-xs">
                  <span className="font-bold uppercase tracking-wider" style={{ color: accent }}>{h.label}:</span>
                  <span style={{ color: textTitle }}>{h.value}</span>
                </div>
              ))}
            </div>
          </div>
        </div>
      </section>

      {/* TIMELINE EXPERIENCE SECTION */}
      {(!sectionVisibility || sectionVisibility.Journey !== false) && (
        <section
          id="experience"
          data-t4-section
          className="py-24 px-6 border-b"
          style={{ borderColor: borderColor }}
        >
          <div className="max-w-4xl mx-auto">
            <h2 className="text-2xl font-black uppercase tracking-wider text-left mb-16" style={{ color: textTitle }}>
              Experience
            </h2>

            <div className="space-y-12">
              {timelineData.map((node, i) => (
                <motion.div
                  key={i}
                  className="grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-8 text-left"
                  initial={{ opacity: 0 }}
                  whileInView={{ opacity: 1 }}
                  viewport={{ once: true }}
                  transition={{ duration: 0.6 }}
                >
                  {/* Left: Duration */}
                  <div className="md:col-span-4 text-xs font-bold font-mono tracking-wider" style={{ color: accent }}>
                    {node.time}
                  </div>

                  {/* Right: Content details */}
                  <div className="md:col-span-8 space-y-2">
                    <h3 className="text-md font-bold text-white leading-tight">
                      {node.title}
                    </h3>
                    <div className="text-xs font-semibold text-stone-500 uppercase tracking-widest font-mono">
                      {node.org} &middot; {node.location}
                    </div>
                    {node.description && (
                      <p className="text-xs leading-relaxed" style={{ color: textColor }}>
                        {node.description}
                      </p>
                    )}
                  </div>
                </motion.div>
              ))}
            </div>
          </div>
        </section>
      )}

      {/* FORMATION CERTIFICATIONS SECTION */}
      {(!sectionVisibility || sectionVisibility.Certification !== false) && (
        <section
          id="formation"
          data-t4-section
          className="py-24 px-6 border-b"
          style={{ borderColor: borderColor }}
        >
          <div className="max-w-6xl mx-auto">
            <h2 className="text-2xl font-black uppercase tracking-wider text-left mb-16" style={{ color: textTitle }}>
              Formation
            </h2>

            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {certifications.map((cert, i) => {
                return (
                  <motion.div
                    key={i}
                    className="p-6 rounded-2xl border text-left flex flex-col justify-between min-h-[160px] transition-all t4-border-glow"
                    style={{
                      backgroundColor: bgCard,
                      borderColor: borderColor
                    }}
                    initial={{ opacity: 0, y: 15 }}
                    whileInView={{ opacity: 1, y: 0 }}
                    viewport={{ once: true }}
                    transition={{ delay: i * 0.08, duration: 0.5 }}
                  >
                    <div className="flex gap-4">
                      <div className="text-2xl shrink-0 mt-0.5" style={{ color: accent }}>
                        <FiAward />
                      </div>
                      <div className="space-y-1">
                        <h4 className="text-xs font-bold text-white uppercase tracking-wider font-mono">
                          {cert.title}
                        </h4>
                        <span className="text-[10px] text-stone-500 block">
                          {cert.issuer}
                        </span>
                      </div>
                    </div>

                    <div className="flex items-center justify-between border-t border-stone-900/40 pt-4 mt-6">
                      <span className="text-[9px] font-mono uppercase tracking-wider text-stone-600">
                        {cert.lastUpdated || "Verified"}
                      </span>
                      {cert.offeredBy && (
                        <span className="text-[9px] font-mono text-stone-400 font-bold">
                          {cert.offeredBy}
                        </span>
                      )}
                    </div>
                  </motion.div>
                );
              })}
            </div>
          </div>
        </section>
      )}

      {/* PROJECTS SECTION */}
      {(!sectionVisibility || sectionVisibility.Projects !== false) && (
        <section
          id="projects"
          data-t4-section
          className="py-24 px-6 border-b"
          style={{ borderColor: borderColor }}
        >
          <div className="max-w-6xl mx-auto">
            <h2 className="text-2xl font-black uppercase tracking-wider text-left mb-16" style={{ color: textTitle }}>
              Projects
            </h2>

            <div className="space-y-6">
              {projects.slice(0, 4).map((proj, i) => {
                return (
                  <motion.div
                    key={i}
                    className="p-6 rounded-2xl border text-left flex flex-col md:flex-row md:items-center justify-between gap-6 transition-all t4-border-glow"
                    style={{
                      backgroundColor: bgCard,
                      borderColor: borderColor
                    }}
                    initial={{ opacity: 0, y: 15 }}
                    whileInView={{ opacity: 1, y: 0 }}
                    viewport={{ once: true }}
                    transition={{ duration: 0.6 }}
                  >
                    <div className="space-y-2">
                      <h4 className="text-sm font-bold text-white uppercase tracking-widest font-mono">
                        {proj.title}
                      </h4>
                      <p className="text-xs max-w-2xl leading-relaxed" style={{ color: textColor }}>
                        {proj.synopsis}
                      </p>
                      <div className="flex flex-wrap gap-2 pt-1.5">
                        {(proj.stacks || []).map((st, idx) => (
                          <span
                            key={idx}
                            className="text-[9px] font-mono font-bold tracking-widest uppercase text-stone-500"
                          >
                            #{st}
                          </span>
                        ))}
                      </div>
                    </div>

                    <div className="flex items-center gap-4 shrink-0">
                      {proj.liveLink && (
                        <a
                          href={proj.liveLink}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-center gap-1.5 border px-4 py-2 rounded-xl text-[10px] font-bold uppercase tracking-wider t4-border-glow transition-all"
                          style={{ borderColor: borderColor, color: accent }}
                        >
                          <span>launch</span>
                          <FiExternalLink />
                        </a>
                      )}
                    </div>
                  </motion.div>
                );
              })}
            </div>
          </div>
        </section>
      )}

      {/* CONTACT LINKS SECTION */}
      {(!sectionVisibility || sectionVisibility.Contact !== false) && (
        <section
          id="contact"
          data-t4-section
          className="py-24 px-6"
        >
          <div className="max-w-4xl mx-auto">
            <div className="text-left space-y-4 mb-16">
              <h2 className="text-4xl font-black uppercase tracking-wider" style={{ color: textTitle }}>
                Contact<span style={{ color: accent }}>.</span>
              </h2>
              <p className="text-xs leading-relaxed max-w-md">
                Contact me or follow my social media links below.
              </p>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {(about.socialLinks || []).map((link, i) => {
                const Icon = SOCIAL_ICONS[link.icon] || FiMail;
                return (
                  <a
                    key={i}
                    href={link.href}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="p-5 rounded-2xl border text-left flex items-center justify-between transition-all t4-border-glow"
                    style={{
                      backgroundColor: bgCard,
                      borderColor: borderColor
                    }}
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-full bg-stone-900 border border-stone-800 flex items-center justify-center text-sm" style={{ color: accent }}>
                        <Icon />
                      </div>
                      <span className="text-[11px] font-bold uppercase tracking-widest font-mono text-white">
                        {link.label}
                      </span>
                    </div>
                    <FiChevronRight className="text-xs text-stone-600" />
                  </a>
                );
              })}
            </div>
          </div>
        </section>
      )}

      {/* FOOTER */}
      <footer className="border-t py-12 px-6" style={{ borderColor: borderColor, backgroundColor: isDark ? "#060a10" : "#f8fafc" }}>
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4 text-xs font-mono">
          <span style={{ color: textSub }}>
            &copy; 2026 {about.name || "Dev"}. All rights reserved.
          </span>
          <button
            onClick={() => downloadPdf(about.resumeUrl || resumePdf, about.resumeFileName || "Resume.pdf")}
            className="flex items-center gap-1.5 border px-4 py-2 rounded-xl text-[10px] font-bold uppercase tracking-wider t4-border-glow transition-all"
            style={{ borderColor: borderColor, color: textTitle, backgroundColor: bgCard }}
          >
            <span>resume.pdf</span>
          </button>
        </div>
      </footer>
    </div>
  );
}
